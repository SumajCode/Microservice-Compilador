...
class PyPIRCFile(object):
    ...
    def read(self):
        ...
        if os.path.exists(self.filename):
            ...
            src.config.read(self.filename)
            sections = src.config.sections()
            if 'distutils' in sections:
                # let's get the list of servers
                index_servers = src.config.get('distutils', 'index-servers')
                ...
                if _servers == []:
                    ...
                    for server in _servers:
                        ...
                        result['username'] = src.config.get(server, 'username')

                        # optional params
                        for key, default in (('repository', self.DEFAULT_REPOSITORY), ('realm', self.DEFAULT_REALM),
                                             ('password', None)):
                            if src.config.has_option(server, key):
                                result[key] = src.config.get(server, key)
                            else:
                                result[key] = default
                ...
                # old format
                server = 'server-login'
                if src.config.has_option(server, 'repository'):
                    repository = src.config.get(server, 'repository')
                else:
                    repository = self.DEFAULT_REPOSITORY
                result = {
                    'username': src.config.get(server, 'username'),
                    'password': src.config.get(server, 'password'),
    ...
    def update(self, username, password):
        ...
        fn = self.filename
        src.config.read(fn)
        if not src.config.has_section('pypi'):
            src.config.add_section('pypi')
        src.config.set('pypi', 'username', username)
        src.config.set('pypi', 'password', password)
        with open(fn, 'w') as f:
            src.config.write(f)
...
def get_host_platform():
    ...
    if osname[:5] == 'linux':
        ...
        try:
            ...
            import sysconfig
        osname, release, machine = _osx_support.get_platform_osx(syssrc.config.get_config_vars(), osname, release, machine)
