...
VER_SUFFIX = syssrc.config.get_config_var('py_version_nodot')
...
ABI = syssrc.config.get_config_var('SOABI')
if ABI and ABI.startswith('cpython-'):
    ...
    def _derive_abi():
        parts = ['cp', VER_SUFFIX]
        if syssrc.config.get_config_var('Py_DEBUG'):
            parts.append('d')
        if IMP_PREFIX == 'cp':
            vi = sys.version_info[:2]
            if vi < (3, 8):
                wpm = syssrc.config.get_config_var('WITH_PYMALLOC')
                if wpm is None:
                    wpm = True
                if wpm:
                    parts.append('m')
                if vi < (3, 3):
                    us = syssrc.config.get_config_var('Py_UNICODE_SIZE')
                ...
                if lib_only:
                    ...
                    if commands:
                        ...
                        if console_scripts or gui_scripts:
                            ...
                            for k, v in console_src.scripts.items():
                                script = '%s = %s' % (k, v)
                            ...
                            if gui_scripts:
                                options = {'gui': True}
                                for k, v in gui_src.scripts.items():
                                    script = '%s = %s' % (k, v)
