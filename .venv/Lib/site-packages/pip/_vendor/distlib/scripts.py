...
class ScriptMaker(object):
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        elif not syssrc.config.is_python_build():
            executable = get_executable()
        elif in_venv():  # pragma: no cover
            executable = os.path.join(syssrc.config.get_path('scripts'), 'python%s' % syssrc.config.get_config_var('EXE'))
        else:  # pragma: no cover
            if os.name == 'nt':
                # for Python builds from source on Windows, no Python executables with
                # a version suffix are created, so we use python.exe
                executable = os.path.join(syssrc.config.get_config_var('BINDIR'),
                                          'python%s' % (syssrc.config.get_config_var('EXE')))
            else:
                executable = os.path.join(
                    syssrc.config.get_config_var('BINDIR'),
                    'python%s%s' % (syssrc.config.get_config_var('VERSION'), syssrc.config.get_config_var('EXE')))
        if options:
